...
//  * @param {number} input.systemLoss - System loss in kPa (user input)
//  * @param {number} input.pipeFrictionLoss - Pipe friction loss in kPa (user input)
//  * @param {number} input.fittingLossKpa - Fitting loss in kPa (calculated separately)
...
//   let baseResult;
//   if (mode === "data") {
//     baseResult = calculateDataMode(flowRateLps, chillerTonnage);
//     baseResult.estimatedDropPa = baseResult.estimatedDropKpa * 1000; // ✅ convert
//     delete baseResult.estimatedDropKpa;
//   } else if (mode === "theoretical") {
//     baseResult = calculateTheoreticalMode(
//       flowRateLps,
//       pipeInnerDiameterMm,
//       fluidDensity,
//       fluidViscosity
...
//   // Use fittingLossKpa if provided, otherwise fallback to 0
//   const fittingLoss = typeof fittingLossPa === "number" ? fittingLossPa : 0;
//   const systemLossVal = typeof systemLoss === "number" ? systemLoss : 0;
//   const pipeFrictionLossVal =
//     typeof pipeFrictionLoss === "number" ? pipeFrictionLoss : 0;

//   // Total pressure drop
//   const totalPressureDropKpa =
//     systemLossVal + pipeFrictionLossVal + fittingLoss;

//   return {
//     ...baseResult,
//     systemLoss: systemLossVal,
//     pipeFrictionLoss: pipeFrictionLossVal,
//     fittingLossKpa: fittingLoss,
//     totalPressureDropKpa: Math.round(totalPressureDropKpa * 100) / 100, // 2 decimals
//   };
// }
...
/**
 * Chiller Pressure Drop Calculator (PA VERSION)
 *
 * Calculates pressure drop across chiller heat exchangers using two methods:
 * 1. Data Mode: Rule-of-thumb estimation based on flow rate and tonnage
 * 2. Theoretical Mode: Darcy-Weisbach equation using flow velocity and pipe size
 *
 * All values are now in **Pa (Pascal)** instead of kPa.
 *
 * @version 1.1.0
 */
...
function calculateChillerPressureDrop(input, mode = "data") {
  ...
  const totalPressureDropPa = systemLoss + pipeFrictionLoss + fittingLossPa;
  ...
  return {
    ...
    pipeFrictionLoss,
    fittingLossPa,
    totalPressureDropPa: Math.round(totalPressureDropPa * 100) / 100, // 2 decimals
    units: {
      pressureDrop: "Pa",
      flowRate: "L/s",
      tonnage: "tons",
...
/**
 * Calculate pressure drop using data mode (rule-of-thumb, now in Pa)
 */
function calculateDataMode(flowRateLps, chillerTonnage) {
  // Pressure drop = 0.4 × flow rate (L/s) in kPa → convert to Pa
...
/**
 * Calculate pressure drop using theoretical mode (Darcy-Weisbach, now in Pa)
 */
function calculateTheoreticalMode(
  flowRateLps,
  pipeInnerDiameterMm,
...
/**
 * Get default fluid properties (unchanged)
 */
function getFluidProperties(fluidType = "water", temperatureC = 25) {
  const properties = {
    water: {
